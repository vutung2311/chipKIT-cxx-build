...
show_usage()
    {
        ...
        echo "USAGE:  $0 [-t <tag> | -?]"
        echo "  -A            Skip building ARM->MIPS Canadian-cross compiler"
        echo "  -G            Enable Graphite loop optimization framework"
        ...
        echo "  -N            No git checkout build only"
        echo "  -Q            Show git checkout (no quiet)"
        echo "  -S            Source only; do not build (do not use with -N)"
        echo "  -X            Build full C++ library"
...
while getopts Ab:FGNt:QSX opt
do
    case "$opt" in
      A)
        echo "Skip ARM Linux build"
        SKIPARMLINUX="yes"
        ;;
      G)
        echo "Enable Graphite"
        SKIPGRAPHITE=""
        ;;
      t)
        TVAL=$OPTARG
      ...
      S)
        ...
        ;;
      X)
        echo "Build full C++ library"
        SUPPORT_HOSTED_LIBSTDCXX="--enable-hosted-libstdcxx"
        ;;
     \?) show_usage ;;
    esac
done
...
# GCC 4.8 needs ISL 0.10, 0.11, or 0.12 if building Graphite
...
GIT_CHIPKIT_CXX_REPO=https://api.github.com/repos/vutung2311/chipKIT-cxx/tarball/master
...
GCC_ISL_URL=ftp://gcc.gnu.org/pub/gcc/infrastructure/isl-0.12.2.tar.bz2
...
if [ "$CHECKOUT" = "yes" ] && [ "x$SKIPGRAPHITE" = "x" ]
then
    echo "Downloading $GCC_ISL_URL"
    echo `date` " Downloading source from $GCC_ISL_URL..."
    if [ -e isl ]
    then
        rm -rf isl
    fi
    curl -L $GCC_ISL_URL | tar jx
    assert_success $? "ERROR: Downloading source from $GCC_ISL_URL"
    mv isl-* isl
    assert_success $? "Normalize isl directory name"
fi


# We can exit here if the user did not want to do a build (source code only)
if [ "$DO_BUILD" = "no" ]
then
    echo `date` " DONE Downloading Source..." >> $LOGFILE
    echo DONE Downloading Source.
    status_update "DONE Downloading Source"
...
# The downloaded part support Makefile may not provide a way to copy over the device info files
# so check for that and update the Makefile if needed.  This wil put the device info files into
# the same places they are located in XC32.
HAS_INSTALL_DEVINFO=`grep install-devinfo Makefile`
if [ "x$HAS_INSTALL_DEVINFO" == "x" ] ; then
    echo -e ' ' >> Makefile
    echo -e 'install-devinfo:' >> Makefile
    echo -e '\tmkdir -p $(DESTROOT)/bin' >> Makefile
    echo -e	'\tcp -r ./device-info-files/* $(DESTROOT)/bin' >> Makefile
    echo -e	'\tmkdir -p $(TGTDESTDIR)' >> Makefile
    echo -e	'\tcp ./device-info-files/xc32_device.info $(TGTDESTDIR)' >> Makefile
    echo -e '\tcp -r ./device-info-files/device_files $(TGTDESTDIR)' >> Makefile
    echo -e '\tmkdir -p $(TGTDESTDIR)/bin' >> Makefile
    echo -e '\tcp -r ./device-info-files/device_files $(TGTDESTDIR)/bin' >> Makefile
    echo -e ' ' >> Makefile
fi
unset HAS_INSTALL_DEVINFO

# Install headers and device info files into cross compiler's install image directory
...
make DESTROOT="$WORKING_DIR/$NATIVEIMAGE/pic32-tools" install-headers install-devinfo
...
if [ "x$LINUX32IMAGE" != "x" ] ; then
    ...
    make DESTROOT="$WORKING_DIR/$LINUX32IMAGE/pic32-tools" install-headers install-devinfo
...
make DESTROOT="$WORKING_DIR/export-image/pic32-tools" install-headers install-devinfo
...
make DESTROOT="$WORKING_DIR/win32-image/pic32-tools" install-headers install-devinfo
...
make DESTROOT="$WORKING_DIR/arm-linux-image/pic32-tools" install-headers install-devinfo
assert_success $? "ERROR: Making headers into cross compiler's arm-linux-image install image directory"

if [ "x$LINUX32IMAGE" != "x" ]; then
    make DESTROOT="$WORKING_DIR/$LINUX32IMAGE" install-headers install-devinfo
...
if [ "x$SKIPNATIVE" == "x" ] ; then
    ...
    if [ "x$SKIPGRAPHITE" == "x" ]; then
        ...
        mkdir ppl
        assert_success $? "ERROR: creating directory $WORKING_DIR/native-build/ppl"

        cd ppl
        echo `date` " Configuring native ppl build in $WORKING_DIR/native-build/ppl..." >> $LOGFILE
        ../../chipKIT-cxx/src48x/ppl/configure --prefix=$WORKING_DIR/native-build/host-libs --disable-shared --enable-static --with-gnu-ld $HOSTMACHINE --target=pic32mx --disable-nls --with-libgmp-prefix=$WORKING_DIR/native-build/host-libs --with-gmp=$WORKING_DIR/native-build/host-libs CXXFLAGS="-fpermissive"
        ...
        mkdir isl
        assert_success $? "ERROR: creating directory $WORKING_DIR/native-build/isl"

        cd isl
        echo `date` " Configuring native isl build in $WORKING_DIR/native-build/isl..." >> $LOGFILE
        ../../isl/configure --prefix=$WORKING_DIR/native-build/host-libs --disable-shared --enable-static --with-gnu-ld $HOSTMACHINE --target=pic32mx --with-gmp-prefix=$WORKING_DIR/native-build/host-libs

        # Make native isl and install it
        echo `date` " Making all in $WORKING_DIR/native-build/isl and installing..." >> $LOGFILE
        make all -j4
        assert_success $? "ERROR: making/installing isl build"
        make install
        assert_success $? "ERROR: making/installing isl build"
...
if [ "x$SKIPLINUX32" == "x" ] ; then
    ...
    if [ "x$LINUX32IMAGE" != "x" ] ; then
        ...
        if [ "x$SKIPGRAPHITE" == "x" ]; then
            ...
            cd ppl
            echo `date` " Configuring linux ppl build in $WORKING_DIR/linux32-build/ppl..." >> $LOGFILE
            ../../chipKIT-cxx/src48x/ppl/configure  --prefix=$WORKING_DIR/linux32-build/linux-libs --disable-shared --enable-static --with-gnu-ld --host=$LINUX32_HOST_PREFIX --target=pic32mx --disable-nls --with-libgmp-prefix=$WORKING_DIR/linux32-build/linux-libs --with-gmp=$WORKING_DIR/linux32-build/linux-libs CXXFLAGS="-fpermissive"

            # Make linux ppl and install it
            ...
            mkdir isl
            assert_success $? "ERROR: creating directory $WORKING_DIR/linux32-build/isl"

            cd isl
            echo `date` " Configuring linux isl build in $WORKING_DIR/linux32-build/isl..." >> $LOGFILE
            ../../isl/configure --prefix=$WORKING_DIR/linux32-build/linux-libs --disable-shared --enable-static --with-gnu-ld $HOSTMACHINE --host=$LINUX32_HOST_PREFIX --target=pic32mx --with-gmp-prefix=$WORKING_DIR/linux32-build/linux-libs

            # Make linux isl and install it
            echo `date` " Making all in $WORKING_DIR/linux32-build/isl and installing..." >> $LOGFILE
            make all -j4
            assert_success $? "ERROR: making/installing isl build"
            make install
            assert_success $? "ERROR: making/installing isl build"
            ...
            USE_PPL="--with-ppl=$WORKING_DIR/linux32-build/linux-libs --with-isl=$WORKING_DIR/linux32-build/linux-libs"
            ...
            ../../chipKIT-cxx/src48x/cloog/configure $BUILDMACHINE --enable-optimization=speed --with-gnu-ld '--with-host-libstdcxx=-static-libgcc -Wl,-Bstatic,-lstdc++,-Bdynamic -lm' --prefix=$WORKING_DIR/linux32-build/linux-libs --host=$LINUX32_HOST_PREFIX --with-gmp-prefix=$WORKING_DIR/linux32-build/linux-libs --with-isl-prefix=$WORKING_DIR/linux32-build/linux-libs --with-ppl=$WORKING_DIR/linux32-build/linux-libs --target=pic32mx --disable-shared --enable-static --disable-shared

            # Make linux cloog and install it
            ...
            cd ..
            USE_CLOOG="--with-cloog=$WORKING_DIR/linux32-build/linux-libs"
        else
            USE_CLOOG="--without-cloog"
            USE_PPL="--without-isl"
        fi
...
if [ "x$SKIPGRAPHITE" == "x" ]; then
    ...
    mkdir isl
    assert_success $? "ERROR: creating directory $WORKING_DIR/win32-build/isl"

    cd isl
    echo `date` " Configuring win32 isl build in $WORKING_DIR/win32-build/isl..." >> $LOGFILE
    ../../isl/configure --prefix=$WORKING_DIR/win32-build/host-libs --disable-shared --enable-static --with-gnu-ld $HOSTMACHINE --host=$MINGW32_HOST_PREFIX --target=pic32mx --with-gmp-prefix=$WORKING_DIR/win32-build/host-libs

    # Make win32 isl and install it
    echo `date` " Making all in $WORKING_DIR/win32-build/isl and installing..." >> $LOGFILE
    make all -j4
    assert_success $? "ERROR: making/installing isl build"
    make install
    assert_success $? "ERROR: making/installing isl build"
    ...
    USE_PPL="--with-ppl=$WORKING_DIR/win32-build/host-libs --with-isl=$WORKING_DIR/win32-build/host-libs"
    ...
    ../../chipKIT-cxx/src48x/cloog/configure $BUILDMACHINE --with-gnu-ld --prefix=$WORKING_DIR/win32-build/host-libs --host=$MINGW32_HOST_PREFIX --target=pic32mx --with-gmp-prefix=$WORKING_DIR/win32-build/host-libs --with-isl-prefix=$WORKING_DIR/win32-build/host-libs --with-ppl=$WORKING_DIR/win32-build/host-libs --target=pic32mx --disable-shared --enable-static --disable-shared

    # Make win32 cloog and install it
    ...
    cd ..
    USE_CLOOG="--with-cloog=$WORKING_DIR/win32-build/host-libs"
else
    USE_CLOOG="--without-cloog"
    USE_PPL="--without-isl"
fi
...
if [ "x$SKIPGRAPHITE" == "x" ]; then
    ...
    # Make arm-linux ppl and install it
    ...
    mkdir isl
    assert_success $? "ERROR: creating directory $WORKING_DIR/arm-linux-build/isl"

    cd isl
    echo `date` " Configuring arm-linux isl build in $WORKING_DIR/arm-linux-build/isl..." >> $LOGFILE
    ../../isl/configure --prefix=$WORKING_DIR/arm-linux-build/host-libs --disable-shared --enable-static --with-gnu-ld $HOSTMACHINE --host=$ARMLINUX32_HOST_PREFIX --target=pic32mx --with-gmp-prefix=$WORKING_DIR/arm-linux-build/host-libs

    # Make arm-linux isl and install it
    echo `date` " Making all in $WORKING_DIR/arm-linux-build/isl and installing..." >> $LOGFILE
    make all -j4
    assert_success $? "ERROR: making/installing isl build"
    make install
    assert_success $? "ERROR: making/installing isl build"
    ...
    USE_PPL="--with-ppl=$WORKING_DIR/arm-linux-build/host-libs --with-isl=$WORKING_DIR/arm-linux-build/host-libs"
    ...
    ../../chipKIT-cxx/src48x/cloog/configure $BUILDMACHINE --with-gnu-ld --prefix=$WORKING_DIR/arm-linux-build/host-libs --host=$ARMLINUX32_HOST_PREFIX --target=pic32mx --with-gmp-prefix=$WORKING_DIR/arm-linux-build/host-libs --with-isl-prefix=$WORKING_DIR/arm-linux-build/host-libs --with-ppl=$WORKING_DIR/arm-linux-build/host-libs --target=pic32mx --disable-shared --enable-static --disable-shared

    # Make arm-linux cloog and install it
    ...
    cd ..
    USE_CLOOG="--with-cloog=$WORKING_DIR/arm-linux-build/host-libs"
else
    USE_CLOOG="--without-cloog"
    USE_PPL="--without-isl"
fi
...
find . -type f -name *-gdb.py | xargs rm -rf
cd ../$NATIVEIMAGE
